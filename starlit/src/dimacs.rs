...
impl<'a> Tokenizer<'a> {
    /// Initializes a [`Tokenizer`] from a [`BufReader`].
    ///
    ...
    /// Initializes a [`Tokenizer`] with an underlying [`Read`] instance.
    ///
    ...
    /// Initializes a [`Tokenizer`] with an underlying boxed [`Read`] instance.
    ///
    /// If the [`Read`] instance is a [`BufReader`], it is better to use
    /// [`from_buf_reader`][Self::from_buf_reader] to avoid unnecessary copying of the read data.
    ///
    ...
    /// Advances to the next token, processing more input when required.
    ///
    ...
    /// Updates the data of the current token.
    ///
    ...
    /// Handles an overlong token that does not fit into the fixed buffer.
    ///
    ...
    /// Processes data `scanned_upto..len` in the fixed buffer
    #[inline(never)]
    fn scan(&mut self) {
